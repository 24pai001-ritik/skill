...
4. **Doc drift** — Code identifiers, library names, model names, or behavior changed but comments / docstrings / READMEs still reference the old version. Also: hard-coded line numbers in comments, stale model/SDK names in prose, numbered/ordered comment lists that drifted from the actual code execution order (comment says rules 1→2→3→4→5 but execution order is 2→1→3→5→4), AND hard-coded counts/enumerations of a collection the code actually sources from a constant (a docstring saying "the four known headings" when they come from a list — the number re-drifts on the next add/remove, so the fix is to remove the count, not correct it).
...
**Re-sweep the fixes the review itself writes.** When a sweep finds something and you edit the code to fix it, that edit is NEW code — run the relevant sweeps over it too before finishing. Fixes are a top source of escaped findings precisely because they feel "done" and skip the gauntlet: a fix that adds a regex can be over-broad (Sweep 1), a fix that corrects a stale count can re-hardcode it (Sweep 4), a fix that adds a guard can miss a sibling teardown (Sweep 3). Treat the post-fix working tree as the diff to audit, not the original one.
  - Example (Copilot, PR english-synonyms-antonyms): BOTH escaped findings were in self-review fixes, not the original change — a `\s*&\s*` speech rule added while fixing a TTS gap (over-matched "AT&T"), and a docstring "three → four" count correction that should have dropped the count. The original diff was swept; its fixes were not.
...
- **Silent exception swallow on a diagnostically-useful path** (no sibling needed): a new `except Exception: pass` / empty `catch {}` / `catch { /* ignore */ }` that drops a failure whose cause you'd want to see in production — an import that can fail, a resolver that can raise, a snapshot/compute that can throw. Even when the failure is genuinely NON-fatal (the feature degrades gracefully), a bare `pass` makes the degradation invisible: the feature silently stops working and no log says why. Flag any new catch-all that neither re-raises nor logs. Fix: keep it non-fatal but VISIBLE — log a warning/debug WITH the exception (`print(f"⚠️ … {e}")` matching the file's sibling handler, or `logger.debug("…", exc_info=True)` on a hot path).
  - Example caught (Copilot, PR classroom-english): a curiosity grade snapshot did `except Exception: pass` — if the grade resolver or its import ever failed, age-targeting silently vanished with zero visibility; a thread-info gating snapshot did the same. Fix: `except Exception as e: print(f"⚠️ [CURIOSITY] … {e}")` and `logger.debug("… failed", exc_info=True)`.
- **Boolean/truthy coercion of a value that's SUPPOSED to be a strict boolean** (`!!x` / `if (x)` where `x` is a boolean-intended field from an external boundary — a backend JSON response, `localStorage`, a query param): `!!x` maps any truthy non-boolean (a string, an object) to `true` and maps `null`/`undefined`/`0`/`""` to `false` — silently losing an explicit `false` and mis-classifying `null` as `false`. When the field is meant to be a boolean, gate on `typeof x === 'boolean'` and assign `x` directly, so an explicit `false` is preserved and a `null`/absent value is skipped (not coerced to `false`). Sibling tell: the SAME field is read with a strict `typeof === 'boolean'` check at one site but `!!`/truthiness at another in the same diff.
  - Example caught (Copilot, PR classroom-english): a resume path did `if (info?.classroom_mode !== undefined) setClassroomMode(!!info.classroom_mode)` while the sibling create/forward path already used `typeof request.classroom_mode === 'boolean'` for the SAME field. Fix: `if (typeof info?.classroom_mode === 'boolean') setClassroomMode(info.classroom_mode)`.
- **Interactive toggle-group missing selected-state semantics for a11y** (lint-adjacent, but automated reviewers flag it): a new set of `<button>`s acting as a single-choice toggle (Heavy/Light, tabs, segmented control) that shows the active option via style ONLY, with no `aria-pressed` (toggle buttons) or `role="radio"` + `aria-checked` (radiogroup) — a screen reader can't announce which option is active. Add `aria-pressed={selected === opt}` to each button (or convert to a radiogroup). Sibling tell: if you copied an existing accessible toggle in the same file, does yours drop its `aria-*`?
  - Example caught (Copilot, PR classroom-english): the Teacher heavy/light card set border/background on the active button but exposed no `aria-pressed`. Fix: `aria-pressed={classroomMode === on}`.
...
- **Unnormalized lookup key (strip/case not applied before a dict/set membership test)**: When an EXTERNAL value (DB row, chapter/plan metadata, config, header, query param) is used as a `dict.get(x)` key or `x in SET` membership test whose keys are normalized (lowercased, trimmed, canonical form), the value must be normalized the SAME way FIRST. External data routinely carries trailing/leading whitespace, mixed case, or NBSP that silently misses the lookup and falls through to a wrong default — the more damaging because the happy-path value (no whitespace) passes every test. The strong tell is INCONSISTENT normalization *within the same function*: one branch does `x.strip().title()` / `(x or "").strip()` but the sibling `LOOKUP.get(x.lower())` / `x.lower() in SET` skips the `.strip()`. Normalize once into a local (`k = x.strip().lower()`) and reuse it for every lookup AND the fallback branches.
  - Method: for each new `dict.get(<expr>)` / `<expr> in <set/dict>` added in the diff whose keys are normalized constants, confirm `<expr>` is stripped + cased to match. Grep the same function for a sibling `.strip()`/`.lower()`/`.title()` on the same variable — if one site normalizes and the lookup doesn't, flag. Test with a whitespace-padded input (`"English Grammar "`), not just the clean literal.
  - Example caught (Copilot, PR english-grammar): `resolve_effective_language` did `subj_lower = subject.lower()` then `_SUBJECT_LANGUAGE_ALIASES.get(subj_lower)`, while the SAME function's other branches used `subject.strip().title()` and `(medium or "English").strip()`. A chapter-meta subject `"English Grammar "` (trailing space) missed the alias and fell back to the medium language — the exact regression the alias existed to prevent. Fix: `subj_clean = subject.strip(); subj_lower = subj_clean.lower()` reused for both the alias `.get` and the `LANGUAGE_SUBJECTS` branch.
...
- **Optional-whitespace separator match catches glued occurrences** (`\s*X\s*` where X also appears INSIDE tokens): a normalize/replace that rewrites a *spaced separator* `X` (` & ` → " and ", ` / ` → pause, ` - ` → range) written with `\s*X\s*` (whitespace OPTIONAL on each side) ALSO fires on `X` glued inside a token — `\s*&\s*` turns "AT&T" → "AT and T" and "R&B" → "R and B"; `\s*-\s*` mangles "e-mail"/"x-ray"; `\s*/\s*` splits "TCP/IP". `\s*` means "zero or more", so the *unspaced* form matches too. If only the spaced-separator form is intended, require whitespace on BOTH sides (`\s+X\s+`), or anchor to the known context.
  - Method: for each new `\s*<char>\s*` (or one-sided `\s*<char>` / `<char>\s*`) replace/split where `<char>` can legally appear glued inside a word / abbreviation / URL / hyphenate, construct a GLUED example (AT&T, e-mail, TCP/IP) and run it. If it gets mangled and only the separator form was intended, switch to `\s+…\s+`. (Sibling of the over-broad-pattern check above: there the shape was too generic; here the *whitespace quantifier* is too loose.)
  - Example caught (Copilot, PR english-synonyms-antonyms): a new `re.sub(r'\s*&\s*', " and ", speech)` added so TTS speaks the "Synonyms & Antonyms" heading also rewrote "AT&T"/"R&B" that can appear in a Key words item. Fix: `\s+&\s+` — whitespace required both sides, so only the heading separator matches, not a glued `&`.
...
**Special check — Conditional hydration leaves stale state on resume / re-init**: On a path that RE-HYDRATES component state from a fetched snapshot (session resume, refresh restore, route re-entry, tab focus refetch), a field written as `if (snapshot.x) setX(mapped(snapshot.x))` is only reset WHEN the snapshot includes `x`. If the snapshot omits `x` (backend didn't send it, returned `null`, older API) the setter never fires and `setX` retains whatever it held from a PRIOR render/session — stale UI state that looks live. The fix is to ALWAYS set from the snapshot with safe defaults (`setX({ a: !!snap?.x?.a, b: !!snap?.x?.b })`) so a missing field resolves to the documented default rather than the previous value. Applies to any state that a later render reads as authoritative (gating flags, feature toggles, counts, selected ids).
  - Method: for each `if (snapshot?.field) setState(...)` ADDED on a resume/re-init path, ask: if `field` is absent on the NEXT resume, does the state keep a stale value from before? If the component isn't guaranteed a fresh mount (state can survive across the re-hydration), unconditionalize the set with defaults.
  - Example caught (Copilot, PR classroom-english): resume did `if (info?.classroom_gating) setClassroomGating({...})` — a resume where the backend omitted the snapshot retained the previous topic's greyed/disabled button state. Fix: `setClassroomGating({ literaryShown: !!info?.classroom_gating?.literary_shown, continueExhausted: !!info?.classroom_gating?.continue_exhausted })` unconditionally.
...
7. **Hardcoded count or re-listing of a collection that's sourced from a single source of truth**: a comment/docstring embeds a literal COUNT ("the three known headings", "all 4 modes", "both flags", "the two callers") or spells out the members of a set that the code actually reads from a named constant / enum / config at runtime. The number (or inline list) re-drifts the moment an item is added to or removed from the source — so even *fixing* the count (three → four) just arms the next drift. This is the doc-side mirror of the Sweep-1 "single source of truth" principle: if the code derives it from `X`, the prose must not re-hardcode it.
   - Method: for each comment/docstring sitting next to a loop / lookup / regex that reads a named constant/enum/collection, scan the prose for an embedded cardinal ("two"/"3"/"four"/"all N"/"both") or an inline re-listing of the members. If the members come from a named source, flag — the fix is to name the source ("the headings in `_LITERARY_HEADINGS`") or drop the number ("the known …"), NOT to correct the count.
   - Example caught (Copilot, PR english-synonyms-antonyms): adding a 4th literary heading changed a docstring from "the three known headings" → "the four known headings", but the headings are sourced from `_LITERARY_HEADINGS`, so any count re-drifts on the next add/remove. Fix: `the known "### " headings (sourced from _LITERARY_HEADINGS)` — no count. (The self-review that changed three→four missed this because it treated the number as a fact to update, not a fact to remove.)
...
- **Log level too high for a hot / newly-frequent path**: an `INFO`/`WARN` log statement that sits inside a per-request handler, a per-chunk callback, a per-turn compute, OR — the common diff shape — a helper that USED to be called once but the diff now ALSO calls from a frequently-hit path (a resume/refresh endpoint that gets polled, an SSE tick). At INFO it inflates log volume/cost and drowns out actionable INFO lines. When you MOVE or SHARE a logging helper into a second, more-frequent caller, re-check its level: downgrade per-item/per-poll detail to `DEBUG` (or rate-limit). Distinct from the silent-swallow check in Sweep 1 (there the problem is too LITTLE logging; here it's too much at too high a level).
  - Method: for each log call ADDED, or each existing log call whose enclosing function GAINED a new caller in the diff, ask "how often does this now fire?" If it's per-request / per-chunk / per-poll and logs routine state (not an error or a once-per-session milestone), flag INFO/WARN → suggest DEBUG.
  - Example caught (Copilot, PR classroom-english): `classroom_gating_snapshot` logged `[CLASSROOM_GATING]` at INFO; after a refactor it was called from BOTH the live SSE path AND the resume/thread-info endpoint (pollable), so the INFO line multiplied on every resume. Fix: `logger.debug(...)`.
...
[HYGIENE] <file>:<line>
  ...
  Issue:       sync-io-in-async | external-state-in-render | heavy-compute-per-chunk | quadratic-loop | read-after-write | same-tab-sync-missing | log-level-too-high
  Risk:        event-loop-stall | stale-UI | UI-jank | quadratic-time | wrong-prev-value | UI-out-of-sync-with-storage | log-volume-inflation
  Fix:         <asyncio.to_thread | useState + storage listener | defer until stream done | chunk the loop | capture-before-update | dispatch custom event | downgrade INFO→DEBUG / rate-limit>
```
